/* Build the single-file game from src/.
   ...
   the pure core can be tested in node; this joins them back together.
     ...
     node tools/assemble.mjs          write graysons-spelling-game.html
...
const OUT = path.join(ROOT, 'graysons-spelling-game.html');

const read = f => fs.readFileSync(path.join(SRC, f), 'utf8');
const STYLES = ['10-app.css', '15-stage.css'];
const SCRIPTS = ['20-core.js', '30-speech.js', '40-sfx.js', '50-art.js', '60-stage.js', '70-app.js'];
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (current !== html) {
      console.error('graysons-spelling-game.html is out of date: run `npm run build`');
      process.exit(1);
    }
    console.log('graysons-spelling-game.html is up to date');
  } else {
