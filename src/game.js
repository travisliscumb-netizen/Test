...
function buildProps(level) {
  ...
  for (var v = 0; v < vatSpots.length; v++) {
    ...
    var gauge = live(MAT.ledGreen, 0.26, 0.26, 0.10, vx + 1.34, 1.7, vz, false);
...
function buildAlarmPanels(level, group) {
  level.panels = [];
  var box = new THREE.BoxGeometry(0.34, 0.42, 0.10);
  var lamp = new THREE.BoxGeometry(0.10, 0.10, 0.04);
...
function startMission() {
  ...
  if (LEVEL.tanks) {
    for (i = 0; i < LEVEL.tanks.length; i++) {
      ...
      LEVEL.tanks[i].gauge.material = MAT.ledGreen;
    }
  }
