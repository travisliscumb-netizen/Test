...
function checkWin() {
  return fieldObjectivesMet() && playerIsAtExit();
}
function checkLoss() {
...
var PANEL_SPOTS = [
  [3.5, 10.0], [12.0, 11.0], [20.5, 12.0],
  [19.0, 17.0], [22.5, 23.0], [8.0, 22.0], [5.0, 26.5]
];
...
function buildAlarmPanels(level, group) {
  ...
  for (var i = 0; i < PANEL_SPOTS.length; i++) {
    var wx = cellToWorldX(PANEL_SPOTS[i][0]), wz = cellToWorldZ(PANEL_SPOTS[i][1]);
...
function spawnEnemies() {
  for (var i = 0; i < ENEMIES.list.length; i++) {
    ...
    // a rifle knocked out of someone's hands lives in the scene, not on the body
    if (old.droppedGun) {
      RENDER.scene.remove(old.droppedGun);
      if (old.droppedGun.geometry) old.droppedGun.geometry.dispose();
      old.droppedGun = null;
    }
...
function updateAimProbe() {
  ...
  var meshes = [];
  for (var i = 0; i < ENEMIES.list.length; i++) {
    if (ENEMIES.list[i].state === 'DEAD') continue;
    meshes.push(ENEMIES.list[i].hitMesh);
  }
  var eh = _aimRay.intersectObjects(meshes, true);
  if (eh.length) {
  ...
  // way. One extra cast, and only when something was actually found.
  var wall = _aimRay.intersectObjects(LEVEL.raycastTargets, false);
  for (i = 0; i < wall.length; i++) {
    var ud = wall[i].object.userData;
    if (ud && (ud.panel || ud.tank || ud.terminal)) continue;   // the fixture itself
    // Geometry you are standing in or against is not "in the way": at a
    // console or up against a rail the nearest hit is centimetres off the
    // camera, and treating that as an occluder would blank the prompt exactly
    // when you are close enough to use the thing.
    if (wall[i].distance < 0.45) continue;
    if (wall[i].distance < bestDist - 0.05) return;             // occluded
    break;
  }
...
var Enemy = function (position, patrolPoints, kind) {
  ...
  this.alarmTarget = null; this.alarmBest = Infinity; this.alarmStall = 0;
...
Enemy.prototype.setState = function (s) {
  ...
  // Raising the alarm is no longer a side effect of noticing you: a guard has
  // to reach a panel and pull it. See alert() and chase().
};
...
var _eyeVec = new THREE.Vector3(), _sightTo = new THREE.Vector3(),
    _shootTo = new THREE.Vector3(), _shootDir = new THREE.Vector3();
...
Enemy.prototype.update = function (dt, playerPos) {
  ...
  if (this.canSee) {
    // owned vector, written in place: this ran every frame for every enemy
...
Enemy.prototype.alert = function (dt) {
  ...
  if (this.stateClock >= CFG.ai.alertPause) {
    if (!this.isCombatant()) { this.setState('SEARCH'); return; }   // staff and the disarmed flee
...
Enemy.prototype.chase = function (dt, playerPos) {
  ...
  if (this.alarmTarget) {
    if (GAME.alarm || !this.alarmTarget.alive) { this.clearAlarmRun(); }
      ...
      var pp = this.alarmTarget.pos;
      var gap = Math.hypot(pp.x - this.group.position.x, pp.z - this.group.position.z);
      // The panel is on a wall, so he pulls it from arm's length rather than
      // walking into it.
      if (gap < 1.7) { raiseAlarm(this); this.clearAlarmRun(); return; }
      // Guards walk straight at what they want; there is no path search here.
      // A runner whose way is blocked by geometry would otherwise stand in a
      // corner forever and the alarm would simply never come, so a run that
      // stops making progress is abandoned and he turns and fights instead.
      if (gap < (this.alarmBest || Infinity) - 0.05) {
        this.alarmBest = gap; this.alarmStall = 0;
      } else {
        this.alarmStall = (this.alarmStall || 0) + dt;
        if (this.alarmStall > 2.5) { this.clearAlarmRun(); }
      }
      if (this.alarmTarget) { this.moveToward(pp, dt, this.chaseSpeed); return; }
    }
  }
...
/* Is this enemy plausibly on the player's screen? Cheap: a facing test first,
   and only then the raycast. Used to decide whether a failsafe may teleport him. */
var _seeChk = null;
...
Enemy.prototype.moveToward = function (target, dt, speed, noArrive) {
  ...
  // Failsafe: an enemy that intends to move but cannot is put back on its patrol
  // node rather than grinding a corner forever. A guard blinking across the room
  // in full view is worse than the stall it fixes, so the short fuse only burns
  // while the player cannot see him; on screen he gets a much longer grace
  // period, and the reset is still guaranteed so nothing can lock up for good.
  if (blockedX && blockedZ) {
    this.stuck = (this.stuck || 0) + dt;
    var onScreen = this.canSee || playerCanSeeEnemy(this);
    if (this.stuck > (onScreen ? 6 : 2)) {
      this.stuck = 0;
...
Enemy.prototype.takeDamage = function (amount, fromPos, zone) {
  ...
  if (zone === 'arm' && this.gun && !this.disarmed) {
    this.disarm();
    this.setState('SEARCH');            // he is running, not fighting
    this.lastKnownPlayerPos = null;
    return;
  }
...
/* Knock the rifle out of a guard's hands and leave it on the floor. */
...
Enemy.prototype.clearAlarmRun = function () {
  ...
  this.alarmBest = Infinity; this.alarmStall = 0;
...
Enemy.prototype.disarm = function () {
  ...
  this.gun.position.set(wp.x, 0.06, wp.z);
  this.gun.rotation.set(Math.PI / 2, randRange(0, Math.PI), 0);
  RENDER.scene.add(this.gun);
  this.droppedGun = this.gun;
  this.gun = null;
  var idx = this.parts.indexOf(this.droppedGun);
...
Enemy.prototype.die = function (zone) {
  ...
  PLAYER.kills++;
  GAME.kills++;
  if (this.kind === 'scientist') GAME.sciKills++;
...
function witnessDeath(victim) {
  ...
  for (var i = 0; i < ENEMIES.list.length; i++) {
    ...
    if (!e.lastKnownPlayerPos) e.lastKnownPlayerPos = new THREE.Vector3();
    e.lastKnownPlayerPos.copy(PLAYER.pos);
    e.setState('ALERT');
  }
}
...
Enemy.prototype.updateDeath = function (dt) {
  ...
  // Bodies used to blink out of existence on a 12s timer, which is a visible
  // pop if you happen to be looking at one. They now settle through the floor
  // over the last second instead. Sinking rather than fading on purpose: every
  // enemy shares one set of materials, so fading one would fade all of them.
...
/* Line of sight runs several times a frame across the whole roster. It used to
   build a Raycaster and a direction vector on every call; both are now reused,
   because allocation in a per-frame path is what a garbage collector notices. */
var _losRay = null, _losDir = null;
function hasLineOfSight(from, to) {
  if (!_losRay) { _losRay = new THREE.Raycaster(); _losDir = new THREE.Vector3(); }
  _losDir.copy(to).sub(from);
  var dist = _losDir.length();
  if (dist < 0.01) return true;
  _losDir.divideScalar(dist);
  _losRay.set(from, _losDir);
  _losRay.near = 0.05; _losRay.far = dist - 0.05;
  return _losRay.intersectObjects(LEVEL.raycastTargets, false).length === 0;
}
...
/* Where the player is currently supposed to be heading. */
...
function hudDrawWaypoint(ctx, w, h) {
  ...
  var m = 46;
  var offscreen = behind || sx < m || sx > w - m || sy < m || sy > h - m;
  sx = clamp(sx, m, w - m); sy = clamp(sy, m, h - m);

  var dist = Math.round(Math.hypot(obj.x - PLAYER.pos.x, obj.z - PLAYER.pos.z));
  ctx.save();
  ctx.globalAlpha = offscreen ? 0.72 : 0.95;
  ctx.strokeStyle = '#ffb128'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.arc(sx, sy, 9, 0, 6.283); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(sx - 14, sy); ctx.lineTo(sx - 5, sy);
  ctx.moveTo(sx + 5, sy); ctx.lineTo(sx + 14, sy); ctx.stroke();
  ctx.font = '11px ui-monospace, Menlo, monospace';
  ctx.fillStyle = '#ffb128'; ctx.textAlign = 'center';
  ctx.fillText(dist + 'm', sx, sy + 24);
  ctx.textAlign = 'left';
  ctx.restore();
}
...
TEST_SUITE.run('Enemy FSM exposes exactly the five live states', function () {
  ...
  // CHASE and ATTACK raise the alarm on a live enemy. Pin the alarm flag high
  // for the duration so the probe cannot siren the building before play, then
...
// Each section ships as its own <script>, so nothing declared in a later one is
// visible here yet, and the parser is free to run a zero-delay timeout between
// two of them. DOMContentLoaded is the first moment every section has run, so
// the last one can add its own cases and the whole suite prints as one block.
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function () { TEST_SUITE.report(); });
} else {
  TEST_SUITE.report();
}
...
function endMission(won, reason) {
  ...
  var objSummary = [], ol = activeObjectives();
  for (var oi = 0; oi < ol.length; oi++) {
    objSummary.push(ol[oi].letter + ': ' + (ol[oi].done() ? 'Yes' : 'No'));
  }
  document.getElementById('endObjA').textContent = objSummary.join('  ');
...
function pauseGame() {
  ...
  // click after resuming is swallowed on any semi-automatic weapon.
  INPUT.fire = false; INPUT.fireLatched = false; INPUT.interact = false;
  exitPointerLock();
  audioSuspend();
...
/* The briefing is written from the same objective table the HUD reads, so a
   player choosing 00 Agent is told what the extra two objectives are before
...
function renderBriefing() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    h.textContent = 'Objective ' + list[i].letter;
...
function bindUI() {
  ...
  if (diff) diff.addEventListener('change', function () {
    GAME.difficulty = clamp(parseInt(diff.value, 10) || 1, 0, 2);
